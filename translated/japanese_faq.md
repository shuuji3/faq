...
### どのTuskyのリリースを使うべきですか？

Tuskyには3種類のリリースがあります。

- メインストリームリリース (mainstream). Unless you've been told otherwise, this is what you want
- ベータリリース (beta)。These are released to testers before every mainstream release
- テストリリース (test)。These are released every time there is a change to Tusky
...
> この機能はTusky 21以上 (Nightlyを含む) で利用できます。Tuskyのバージョンは「About」メニューを開くと確認できます。
...
### ダイレクトメッセージはどうすれば送れますか？

> 重要: Mastodon上のダイレクトメッセージは暗号化されていないため、インスタンスの管理者は読むことが可能です。Mastodon上での個人情報の共有には十分注意してください。

- 新しい投稿を作成する
- 送信先のユーザーに@メンションを行う
- "visibilty" アイコンを選択する (デフォルトでは地球アイコンが表示されます)
- 投稿のvisibilityを「ダイレクト」に設定する
